...
![header](https://github.com/celo-org/docs/raw/main/src/data-tutorials/showcase/intermediate/how-to-create-deploy-and-mint-your-erc1155-token-on-celo-with-hardhat.png)
...
2. Go to the terminal of your work environment and run the command

```bash
 npm init -y
```

This will initialize the package manager and create a `package.json` file in preparation before installing hardhat.

3. Next, run the command
   ```bash
   npm install hardhat @nomiclabs/hardhat-waffle @nomicfoundation/hardhat-toolbox @openzeppelin/contracts @nomiclabs/hardhat-waffle
   ```

. also, run the command

```bash
 npm i hardhat-deploy dotenv
```
...
**_Note: Do not copy the code above into your contract file since you can import it directly from openzeppelin instead_**.
...
**_Note: Your wallet MNEMONICS is the same as the key phrase given to you when you initially signed up for an account on your wallet, Its usually ranges from 12 to 18 random words for verifying your ownership like a passphrase_**.

3. Paste the `MNEMONIC` phrase as the value for the variable `MNEMONIC` inside your `.env` file.

**_Note: Your `MNEMONICS` should be kept secret and secure (Not public) at all points. DO NOT REVEAL this phrase at all times. No quotes should be added_**.
...
**_Note: If your statement returns `compiles 3 solidity files successfully`, you’re still on track_**.
...
**_Note: Copy the contract address returned and save it, you will need it to interact with your deployed contract later_**.
...
- To begin interacting with your token contract on the Celo blockchain, run the command

```bash
 npx hardhat console
```

. You should notice a prompt-like arrow `>` to input any statement.

- Now copy and paste the line of code

```javascript
const MyToken = await ethers.getContractFactory("MyToken");
```

and click enter to get the contract factory and assign it to the variable `MyToken`.

- Next copy and paste the line

```javascript
const token = await MyToken.attach(
  "0x8B34A555B17dCeA818C4C69b19f4A4145413B2f3"
);
```
...
**_Note: you should use the address of your deployed contract, the same address that was returned when you deployed the contract_**.
...
b. To check the balance of any of the tokens enter the lines of code

```javascript
await token.balanceOf("0x9A11d5a704038BF7735dCE5Bbc71621da7052395", 0);
```

for the first token or

```javascript
await token.balanceOf("0x9A11d5a704038BF7735dCE5Bbc71621da7052395", 1);
```

for the second token. And you should have a similar result to the image below.

![balance_of](https://user-images.githubusercontent.com/69092079/212209653-50818645-6df6-499b-b6cf-41ff07ac3ed8.jpg)

**_Note: you should pass in the address of the account you used to sign and deploy the contract, which is also the owner of the contract, and the token address, the `msg.sender` address_**.

c. To send your tokens to any wallet address you can call the `safeTransferFrom` function, and pass in the following input. `from` address, `to` address, the `token_id` of the token you want to send, the `amount` you want to send and for the data put in the value `0x00`. For example

```javascript
await token.safeTransferFrom(
  "0x9A11d5a704038BF7735dCE5Bbc71621da7052395",
  "0x8056c7C6B8a3a81bc4CbAf317973ce7f9Cbc747F",
  0,
  50,
  0x00
);
...
d. You can check the remaining balance of the token and you’ll notice the reduction in the token amount. Run the command
